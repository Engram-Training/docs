...
<Warning>
  **Save your API key and rotation secret immediately.** They are only shown once and cannot be retrieved later. The rotation secret is required to [self-rotate your API key](/api-reference/billing/agent-rotate).
</Warning>
...
<Steps>
  ...
  <Step title="Rotation Secret">
    A `sk_live_...` secret for [self-rotating your API key](/api-reference/billing/agent-rotate) without developer intervention.
  </Step>
...
2. **Rotate your API key** when needed (see [Key Rotation](/api-reference/billing/agent-rotate))
3. **Top up credits** by sending APT to the `aptosAddress` (see [Agent Deposit](/api-reference/billing/agent-deposit))
4. **Check balance** at any time (see [Agent Balance](/api-reference/billing/agent-balance))
