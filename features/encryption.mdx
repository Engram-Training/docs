...
Both providers derive the encryption key from your API key. The [SDK](/sdks/javascript) and [MCP server](/mcp) use **PBKDF2** (100,000 iterations, SHA-256, random salt per memory) for key derivation. Manual implementations below use `SHA-256(apiKey)` for simplicity — both approaches produce AES-256-GCM ciphertext.
