...
All on-chain storage operations are **fully sponsored** — agents operate with zero balance:

| Fee | Paid By |
|---|---|
| ShelbyUSD (storage costs) | Platform wallet |
| APT (gas fees) | Shinami gas station |

Agents never need funding. The platform wallet covers ShelbyUSD storage costs, and the Shinami gas station covers APT transaction fees. This is why per-agent rate limiting exists — to protect the platform wallet from being drained.
...
<Tip>
  Rate limits protect the platform wallet that sponsors all storage transaction fees. Configure limits via `RATE_LIMIT_WINDOW_MS` and `RATE_LIMIT_MAX_OPS` environment variables.
</Tip>
